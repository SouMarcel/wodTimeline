...
class Cla(models.Model):
    ...
    historia = models.TextField("História")  # História do clã
    mote = models.CharField("Mote", max_length=200)  # Mote ou lema do clã
    aparencia = models.TextField("Aparência")  # Descrição da aparência característica dos membros do clã
    refugio = models.TextField("Refúgio")  # Descrição do refúgio ou local de residência do clã
    criacao_personagens = models.TextField("Criação de Personagens")  # Diretrizes para a criação de personagens do clã
    organizacao = models.TextField("Organização")  # Estrutura organizacional do clã
    estereotipos = models.TextField("Estereótipos")  # Estereótipos associados ao clã
